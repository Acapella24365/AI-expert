    ...
    while True:
        ...
        if choice == "1":
            if basic_caption.startswith("[Error]"):
                print(f"{Fore.RED}X Caption (5 words): {Style.BRIGHT}{basic_caption}\n")
            else:
            ...
            if basic_caption.startswith("[Error]"):
                print(f"{Fore.RED}X Failed to generate description: {basic_caption}")
                continue
            prompt = ("Rewrite as EXACTLY 30 words. Single paragraph. One complete sentence."
                      
                      " End with a perion. No title/bullets.\n\nText: " + basic_caption)
            try:
