...
mp_draw = mp.solutions.drawing_ults
...
while True:
    ...
    if not success:
        print("Failed to read frame webcam.")
        break
    ...
    if results.multi_hnad_landmarks and results.multi_handedness:
        for i, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            index_pos = (int(thumb_tip.x * w), int(thumb_tip.y * h))
            ...
            if hand_label == "Right":
                ...
                cv2.putText(img, f'Brightess: {int(brightness)}%', (90, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 3)
            
    cv2.imshow("Gesture Volume and Brightess Controller", img)
