...
def temperature_prompt_activivty():
    print("=" * 70)
    print("ADVANCED PROMPT ENGINEERING: TEMPERATURE + INSTRUCTIONS")
    print("=" * 70)

    print("\nPART 1: TEMERATURE EXPLORATION")
    ...
    prompts = [
        ...
        f"Create a fictional news headline 2050 about {topic}.",
    ]
    for i, p in enumerate(prompts, 1):
        ...
        try:
            temp = float(input("Set tempearture (0.1 to 1.0): ").strip())
        ...
        print("\nCHALLENFE: Create a prompt chain:")
        print("Generate contenr -> rewrite with constraints -> create a sequel (try a different temps).")
    ...
    def bonus_stream():
        ...
        if choice == "y":
            ...
            out = generate_response(p, tempearture=0.7, max_tokens=512)
            print("\nStreaming-like response (not real streaming):")
            pseudo_stream(out)

if __name__ == "__main__":
    temperature_prompt_activivty()
    bonus_stream()